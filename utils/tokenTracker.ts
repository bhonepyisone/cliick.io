...
class TokenTracker {
    ...
    private pricing: Record<string, TokenPricing> = DEFAULT_PRICING;
    ...
    async logUsage(params: {
        ...
        userId?: string;
        conversationId?: string;
        operationType: TokenUsageLog['operationType'];
        modelName: string;
        inputTokens: number;
        outputTokens: number;
        metadata?: TokenUsageLog['metadata'];
     ...
     */
    private async saveToSupabase(log: TokenUsageLog): Promise<void> {
    ...
    async loadFromSupabase(
        ...
        return (data || []).map(row => ({
            ...
            operationType: row.operation_type as TokenUsageLog['operationType'],
     ...
     */
    getLogsByShop(shopId: string, limit?: number): TokenUsageLog[] {
     ...
     */
    getLogsByDateRange(startDate: number, endDate: number): TokenUsageLog[] {
     ...
     */
    getLogsByOperation(operationType: TokenUsageLog['operationType']): TokenUsageLog[] {
     ...
     */
    getTotalCostForShop(shopId: string, dateRange?: { start: number; end: number }): number {
     ...
     */
    getTotalTokensForShop(shopId: string, dateRange?: { start: number; end: number }): {
        inputTokens: number;
        outputTokens: number;
        totalTokens: number;
    } {
     ...
     */
    getAverageTokensPerOperation(operationType: TokenUsageLog['operationType']): {
        avgInput: number;
        avgOutput: number;
        avgTotal: number;
        avgCost: number;
     ...
     */
    clearLogsForShop(shopId: string): void {
     ...
     */
    updatePricing(modelName: string, pricing: Omit<TokenPricing, 'modelName'>): void {
     ...
     * Get current pricing
     */
    getPricing(): Record<string, TokenPricing> {
        return { ...this.pricing };
    }
}
