...
import {
    ...
    DataHistoryTier,
    DataHistoryTierConfig,
    AIDescriptionGeneratorConfig,
    AIPhotoStudioConfig,
    AIConversationalCommerceConfig,
    AIModelAssignments,
    AIGlobalConfig,
    PlanEntitlements,
    PlanFeatures,
 ...
 */
export const savePlatformSettings = async (settings: PlatformSettings): Promise<boolean> => {
    try {
        // Get the current session
        const { data: { session }, error: sessionError } = await supabase.auth.getSession();
        if (sessionError || !session) {
        ...
        const { plans: growthPlans, entitlements: growthEntitlements } = generateGrowthPlanTiers(
            settings.growthPlanTierConfig,
        ...
        const { error } = await supabase.functions.invoke('admin-platform-settings', {
...
export const getDefaultPlatformSettings = (): PlatformSettings => {
