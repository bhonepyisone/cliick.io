...
class TokenBudgetService {
    constructor() {
        this.DEFAULT_DAILY_BUDGET = 5.00; // $5 USD
        this.DEFAULT_MONTHLY_BUDGET = 100.00; // $100 USD
        this.AVERAGE_COST_PER_REQUEST = 0.0001; // Estimated
    }
    /**
     * Get or create budget for a shop
     */
    async getShopBudget(shopId) {
     ...
     */
    async createShopBudget(shopId, customBudget) {
        const { data, error } = await client_1.supabase
            .from('shop_token_budgets')
            .insert({
            shop_id: shopId,
     ...
     * Update shop budget
     */
    async updateShopBudget(shopId, updates) {
        const updateData = {};
        if (updates.dailyBudget !== undefined)
            updateData.daily_budget = updates.dailyBudget;
        ...
        if (updates.autoOptimizationEnabled !== undefined)
            updateData.auto_optimization_enabled = updates.autoOptimizationEnabled;
        if (updates.fallbackModel !== undefined)
            updateData.fallback_model = updates.fallbackModel;
        const { data, error } = await client_1.supabase
            .from('shop_token_budgets')
            .update(updateData)
            .eq('shop_id', shopId)
     ...
     */
    async canMakeRequest(shopId, estimatedCost = this.AVERAGE_COST_PER_REQUEST) {
     ...
     */
    async getOptimizationRecommendations(shopId) {
     ...
     */
    async applyOptimization(shopId, operationType, currentConfig) {
        const budget = await this.getShopBudget(shopId);
        const status = await this.getBudgetStatus(shopId);
    ...
    async resetDailyBudgets() {
        const { error } = await client_1.supabase.rpc('reset_daily_token_budgets');
        if (error) {
            console.error('[BUDGET] Error resetting daily budgets:', error);
        }
    }
    async resetMonthlyBudgets() {
        const { error } = await client_1.supabase.rpc('reset_monthly_token_budgets');
        if (error) {
...
// Singleton instance
exports.tokenBudgetService = new TokenBudgetService();
