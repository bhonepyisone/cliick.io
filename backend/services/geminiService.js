...
exports.editProductImageWithModel = exports.generateProductDescriptionsFromModel = exports.generateSuggestionFromModel = exports.generateChatResponse = void 0;
const genai_1 = require("@google/genai");
const platformSettingsService_1 = require("./platformSettingsService");
const shopService = require("./shopService");
const localizationData_1 = require("../data/localizationData");
const tokenTracker_1 = require("../utils/tokenTracker");
const tokenLimiter_1 = require("../utils/tokenLimiter");
const tokenBudgetService_1 = require("./tokenBudgetService");
const pricingService_1 = require("./pricingService");
let ai = null;
if (process.env.API_KEY) {
    ai = new genai_1.GoogleGenAI({ apiKey: process.env.API_KEY });
}
const createConversationalOrderTool = {
...
const createBookingTool = {
    ...
    parameters: {
        type: genai_1.Type.OBJECT,
        properties: {
            customerName: { type: genai_1.Type.STRING, description: 'The full name of the customer.' },
            phoneNumber: { type: genai_1.Type.STRING, description: 'The contact phone number for the customer.' },
            serviceName: { type: genai_1.Type.STRING, description: 'The name of the service to book. Must match a service from the item catalog.' },
            appointmentDate: { type: genai_1.Type.STRING, description: 'The requested date for the appointment (e.g., "Tomorrow", "July 25th", "2024-08-15").' },
            appointmentTime: { type: genai_1.Type.STRING, description: 'The requested time for the appointment (e.g., "around noon", "14:30").' },
        },
        required: ['customerName', 'phoneNumber', 'serviceName', 'appointmentDate', 'appointmentTime'],
    },
};
const generateChatResponse = async (shopId, conversationId, modelName, globalConfig, history, newMessage, assistantConfig, knowledgeBase, language, tone, paymentMethods, userName) => {
    if (!ai) {
        throw new Error("API key not configured.");
    }
    try {
        // Check budget before making request
        const budgetCheck = await tokenBudgetService_1.tokenBudgetService.canMakeRequest(shopId);
        if (!budgetCheck.allowed) {
            // Log billing event for rejected request
            await pricingService_1.pricingService.getRealTimeBillingInfo(shopId).catch(err => console.warn('[GEMINI] Failed to get billing info for rejected request:', err));
            return {
        ...
        const optimization = await tokenBudgetService_1.tokenBudgetService.applyOptimization(shopId, 'chat_message', {
            modelName,
        ...
        const platformSettings = (0, platformSettingsService_1.getPlatformSettings)();
        const shop = shopService.getShopById(shopId);
        if (!shop) {
            throw new Error("Shop not found.");
        }
        const langNameMap = localizationData_1.allLanguages.reduce((acc, lang) => {
...
`;
        // --- NEW STRATEGY LOGIC ---
        const hasPhysicalLocations = (knowledgeBase.userDefined || []).some(section => section.type === 'location_list' && section.locations && section.locations.length > 0);
        ...
        let salesStrategy = 'Informational-Only';
...
`;
        ...
        if (aiPermissions.allowTrainingData) {
            const textSections = (knowledgeBase.userDefined || [])
                ...
                .join('\n\n');
            if (textSections)
                knowledgeForPrompt += textSections;
            ...
            if (locationSections.length > 0) {
                ...
                locationSections.forEach(section => {
                    ...
                    section.locations.forEach(loc => {
                        ...
                        if (loc.phone)
                            locationKnowledge += `  **Phone**: ${loc.phone}\n`;
                        if (loc.operatingHours)
                            locationKnowledge += `  **Hours**: ${loc.operatingHours}\n`;
                        if (loc.notes)
                            locationKnowledge += `  **Notes**: ${loc.notes}\n`;
                        locationKnowledge += '\n';
                    });
                });
...
${knowledgeForPrompt}`;
        ...
        const userMessageContent = { role: 'user', parts: [{ text: newMessage }] };
        ...
        if (usageMetadata) {
            ...
            // Log token usage
            tokenTracker_1.tokenTracker.logUsage({
                shopId,
            ...
            // Check token limits
            const messageLimitViolation = tokenLimiter_1.tokenLimiter.checkMessageLimits('chat_message', inputTokens, outputTokens);
        ...
        if (functionCalls && functionCalls.length > 0) {
            ...
            if (call.name === 'createConversationalOrder') {
                ...
                result = await shopService.createConversationalOrder(shopId, conversationId, call.args);
            }
            else if (call.name === 'createBookingTool') {
                toolName = 'createBookingTool';
                result = await shopService.createConversationalBooking(shopId, conversationId, call.args);
            }
            else {
            ...
            const contentsForToolResponse = [...contents, modelResponseContent, toolResponseContent].filter((c) => !!c);
...
const generateSuggestionFromModel = async (modelName, globalConfig, prompt, systemInstruction) => {
    if (!ai) {
        throw new Error("API key not configured.");
    }
    try {
...
exports.generateSuggestionFromModel = generateSuggestionFromModel;
const generateProductDescriptionsFromModel = async (modelName, globalConfig, prompt, characterLimit) => {
    if (!ai) {
        throw new Error("API key not configured.");
    }
    try {
...
exports.generateProductDescriptionsFromModel = generateProductDescriptionsFromModel;
const editProductImageWithModel = async (modelName, base64Data, mimeType, prompt) => {
    if (!ai) {
        throw new Error("API key not configured.");
    }
    try {
...
exports.editProductImageWithModel = editProductImageWithModel;
