...
router.get('/', async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    const { data, error } = await supabase.from('forms').select('*').eq('shop_id', shopId);
...
router.put('/:formId', authenticateToken, async (req: Request, res: Response, next: NextFunction) => {
  const { shopId, formId } = req.params as any;
  const { name, description } = req.body;
  try {
    const { data, error } = await supabase.from('forms').update({ name, description, updated_at: new Date().toISOString() }).eq('id', formId).eq('shop_id', shopId).select().single();
    if (error) throw error;
...
router.delete('/:formId', authenticateToken, async (req: Request, res: Response, next: NextFunction) => {
  const { shopId, formId } = req.params as any;
  try {
    const { error } = await supabase.from('forms').delete().eq('id', formId).eq('shop_id', shopId);
    if (error) throw error;
