...
router.get('/', async (req, res, next) => {
    try {
        ...
        const { data, error } = await supabase_1.supabase.from('items').select('*').eq('shop_id', shopId);
        if (error)
            throw error;
...
router.get('/:productId', async (req, res, next) => {
    try {
        ...
        const { data, error } = await supabase_1.supabase.from('items').select('*').eq('id', productId).eq('shop_id', shopId).single();
        if (error || !data) {
